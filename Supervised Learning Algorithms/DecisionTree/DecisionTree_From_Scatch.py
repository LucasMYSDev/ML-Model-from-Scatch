import numpy as np
from collections import Counter
from sklearn.preprocessing import LabelEncoder

class Node:
    """
    A Node class for the Decision Tree structure.

    Parameters:
    - feature: int, default=None
      The index of the feature used for splitting at this node.
    - threshold: float, default=None
      The threshold value used for splitting at this node.
    - left: Node, default=None
      The left child node.
    - right: Node, default=None
      The right child node.
    - value: object, default=None
      The class label value if the node is a leaf node.

    Returns:
    None
    """

    def __init__(self, feature=None, threshold=None, left=None, right=None, *, value=None):
        self.feature = feature
        self.threshold = threshold
        self.left = left
        self.right = right
        self.value = value

    def is_leaf_node(self):
        """
        Check if the node is a leaf node.

        Returns:
        - is_leaf: bool
          True if the node is a leaf node, False otherwise.
        """
        return self.value is not None


class DecisionTree:
    """
    Decision Tree classifier implementation for classification tasks.
    """

    def __init__(self, min_samples_splits=2, max_depth=100, n_features=None):
        """
        Initialize the Decision Tree classifier.

        Parameters:
        - min_samples_splits: int, default=2
          The minimum number of samples required to split an internal node.
        - max_depth: int, default=100
          The maximum depth of the decision tree.
        - n_features: int or None, default=None
          The number of features to consider when looking for the best split.
          If None, all features will be considered.

        Returns:
        None
        """
        self.min_samples_splits = min_samples_splits
        self.max_depth = max_depth
        self.n_features = n_features
        self.root = None

    def fit(self, X, y):
        """
        Fit the Decision Tree model to the training data.

        Parameters:
        - X: ndarray, shape (n_samples, n_features)
          The input training data.
        - y: ndarray, shape (n_samples,)
          The target labels.

        Returns:
        None
        """
        self.n_features = X.shape[1] if not self.n_features else min(X.shape[1], self.n_features)
        self.root = self._grow_tree(X, y)

    def _grow_tree(self, X, y, depth=0):
        """
        Recursively grow the decision tree by splitting at each node.

        Parameters:
        - X: ndarray, shape (n_samples, n_features)
          The input training data at the current node.
        - y: ndarray, shape (n_samples,)
          The target labels at the current node.
        - depth: int, default=0
          The current depth of the decision tree.

        Returns:
        - node: Node
          The current node of the decision tree.
        """
        n_samples, n_feats = X.shape
        n_labels = len(np.unique(y))

        if depth >= self.max_depth or n_labels == 1 or n_samples < self.min_samples_splits:
            leaf_value = self.most_common_label(y)
            return Node(value=leaf_value)

        feat_idxs = np.random.choice(n_feats, self.n_features, replace=False)
        best_feature, best_thresh = self._best_split(X, y, feat_idxs)

        left_idxs, right_idxs = self._get_index_split(X[:, best_feature], best_thresh)

        left = self._grow_tree(X[left_idxs, :], y[left_idxs], depth + 1)
        right = self._grow_tree(X[right_idxs, :], y[right_idxs], depth + 1)

        return Node(best_feature, best_thresh, left, right)

    def most_common_label(self, y):
        """
        Find the most common label in the target labels.

        Parameters:
        - y: ndarray, shape (n_samples,)
          The target labels.

        Returns:
        - value: object
          The most common label value.
        """
        counter = Counter(y)
        value = counter.most_common(1)[0][0]
        return value

    def _best_split(self, X, y, feat_idxs):
        """
        Find the best feature and threshold for splitting at the current node.

        Parameters:
        - X: ndarray, shape (n_samples, n_features)
          The input training data at the current node.
        - y: ndarray, shape (n_samples,)
          The target labels at the current node.
        - feat_idxs: ndarray
          The indices of the features to consider for splitting.

        Returns:
        - split_idx: int
          The index of the best feature for splitting.
        - split_threshold: float
          The threshold value for splitting.
        """
        best_gain = -1
        split_idx, split_threshold = None, None

        for feat_idx in feat_idxs:
            X_column = X[:, feat_idx]
            thresholds = np.unique(X_column)

            for thr in thresholds:
                gain = self._information_gain(y, X_column, thr)
                if gain > best_gain:
                    best_gain = gain
                    split_idx = feat_idx
                    split_threshold = thr

        return split_idx, split_threshold

    def _information_gain(self, y, X_column, threshold):
        """
        Calculate the information gain for a split.

        Parameters:
        - y: ndarray, shape (n_samples,)
          The target labels at the current node.
        - X_column: ndarray
          The values of a single feature at the current node.
        - threshold: float
          The threshold value for splitting.

        Returns:
        - information_gain: float
          The calculated information gain for the split.
        """
        parent_entropy = self._entropy(y)

        left_idxs, right_idxs = self._get_index_split(X_column, threshold)

        n = len(y)
        n_l, n_r = len(left_idxs), len(right_idxs)
        e_l, e_r = self._entropy(y[left_idxs]), self._entropy(y[right_idxs])

        child_entropy = (n_l / n) * e_l + (n_r / n) * e_r

        information_gain = parent_entropy - child_entropy
        return information_gain

    def _entropy(self, y):
        """
        Calculate the entropy of the target labels.

        Parameters:
        - y: ndarray, shape (n_samples,)
          The target labels.

        Returns:
        - entropy: float
          The calculated entropy of the target labels.
        """
        label_encoder = LabelEncoder()
        y_encoded = label_encoder.fit_transform(y)
        hist = np.bincount(y_encoded.astype(int))

        ps = hist / len(y)
        return -np.sum([p * np.log(p) for p in ps if p > 0])

    def _get_index_split(self, X_column, split_threshold):
        """
        Get the indices for splitting the data based on a threshold.

        Parameters:
        - X_column: ndarray
          The values of a single feature at the current node.
        - split_threshold: float
          The threshold value for splitting.

        Returns:
        - left_idxs: ndarray
          The indices for the left split.
        - right_idxs: ndarray
          The indices for the right split.
        """
        left_idxs = np.argwhere(X_column <= split_threshold).flatten()
        right_idxs = np.argwhere(X_column > split_threshold).flatten()

        return left_idxs, right_idxs

    def predict(self, X):
        """
        Predict the class labels for the input data.

        Parameters:
        - X: ndarray, shape (n_samples, n_features)
          The input data to be classified.

        Returns:
        - predictions: ndarray, shape (n_samples,)
          The predicted class labels for the input data.
        """
        return [self._traverse_tree(x, self.root) for x in X]

    def _traverse_tree(self, x, node):
        """
        Perform the prediction for a single sample.

        Parameters:
        - x: ndarray, shape (n_features,)
          The input sample to be classified.
        - node: Node
          The current node in the decision tree.

        Returns:
        - predicted_label: object
          The predicted class label for the input sample.
        """
        if node.is_leaf_node():
            return node.value

        if x[node.feature] <= node.threshold:
            return self._traverse_tree(x, node.left)

        return self._traverse_tree(x, node.right)





        
